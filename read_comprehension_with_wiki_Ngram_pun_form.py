...
class extractAnswer:
    ...
    def sorting(self, article, question, topic):
        processes = [mp.Process(target=self.wiki_passage_api, args=(article[x],article, output)) for x in range(len(article))]
        for p in processes:
            p.start()
        for p in processes:
            p.join(timeout=3)
   ...
   #             tf_idf+=(i[j]*idf[j])/(0.3361+(1-0.3361)*((1+log(pass_len[n]))/(1+log(sum(pass_len)/len(pass_len)))))
    ...
    def passage_pre(self, passage):
        ...
        if 'redirects' in passage[0]:
            passage=passage[1] if len(passage)>1 else ' '.join(passage)
        else:
            passage=' '.join(passage)
        passage=re.sub('\n', ' ', passage)
    ...
    def wiki(self, question, topic):
        ...
        question=re.sub('[{0}]'.format(self.symbol), ' ', question).strip()
        ...
        if self.noun:
            if len(self.noun)==2 and len(" ".join(self.noun).split())<6:
                ...
                if self.noun[0] in stop:
                    self.noun.pop(0)
                self.noun=question[question.index(self.noun[0]):question.index(self.noun[-1])+len(self.noun[-1])+1].split()
                print(self.noun)
        ...
        passage1, passage2, article_title, tfidf1, tfidf2=self.sorting(article_list, question, topic)
        if passage1:
...
def ans(question, topic='', passage=''):
    start=time.time()
    if not question:
    ...
    if passage:
        ...
        if answer:
        #return {'Status':'S', 'Answer':answer, 'Time':'{} seconds'.format(round(end-start, 3)), 'Article from wikipedia':title}
            return {'Question':question,'Answer':answer, 'Time taken':'{} seconds'.format(round(end-start, 3))}
    
        else:
            #return {'Status':'E', 'Error message':"Sorry could not find the answer. Please rephrase or ask something else.", "Time":'{} seconds'.format(round(end-start, 3))}
            return {'Status':'E', 'Error message':"Sorry could not find the answer. Please rephrase or ask something else."}
    answer, title=extractor.wiki(question, topic)
    end=time.time()
    if answer:
        #return {'Status':'S', 'Answer':answer, 'Time':'{} seconds'.format(round(end-start, 3)), 'Article from wikipedia':title}
        return {'Question':question,'Answer':answer, 'Article from wikipedia':title, 'Time taken':'{} seconds'.format(round(end-start, 3))}
    ...
    if request.method == 'POST':
        ...
        for i, j in result.items():
            if i=='question':
                ...
                passage=j
        result=ans(question, topic, passage)
        print(result)
...
if __name__ == "__main__":
    http_server = WSGIServer(('0.0.0.0', 7090), app)
